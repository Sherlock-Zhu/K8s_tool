...
letter_codes = [ord(ch) for ch in 'rqblsWdpSD'] + [258, 259, 10]
actions = ['restart', 'exit', 'basic', 'log', 'service', 'switch', 'deployment', 'pod', 'statefulset', 'daemonset'] + ['down', 'up', 'enter']
...
def get_cmd(dict_key, content = None):
    return cmd_dict[dict_key] + ' ' + content if content else cmd_dict[dict_key]
...
def main(stdscr):
    ...
    class workField:
        def __init__(self):
            ...
            self.page_state = ['pod','deployment', 'statefulset', 'daemonset', 'service']
            self.page_normal = ['pod_basic', 'container_list', 'log']
            self.page_other = ['ns']
            self.page_level_1 = ['ns']
            self.page_level_2 = ['pod','deployment', 'statefulset', 'daemonset', 'service']
        ...
        #define print solution
            ...
            def format_output(content):
                format_str = ''
                for i in resp_dict[self.current_page][2:]: format_str += '{:<' + str(i) + '.' + str(i-1) + '}'
                return format_str.format(*[i.strip() for i in filter(None, content.split('  '))][:resp_dict[self.current_page][1]])
            ...
            if self.current_page in self.page_state:
                ...
                for i in range(self.current_firstline, self.current_endline): 
                    if i == self.current_row_no: 
                        cast(format_output(self.content[i]), curses.color_pair(1)) 
                    elif i in issue_line and self.cstate == True: 
                        cast(format_output(self.content[i]), curses.color_pair(2))
                    else:
    ...
    class ActResp:
        ...
        def enter(self):
            ...
            if work_field.current_page == 'container_list':
                ...
                cmd = ' '.join([get_cmd('log'), '-n', work_field.ns, work_field.current_pod, current_con])
        ...
        def basic(self):
            if work_field.current_page == 'pod':
                ...
                #get pod info
                cmd_pod = ' '.join([get_cmd('get', 'pod'), '-o json -n', work_field.ns, work_field.current_pod])
                ...
                cmd_res_use = ' '.join([get_cmd('top'), 'pod -n', work_field.ns, work_field.current_pod])
                # resource requested by described in node
                cmd_res_all = ' '.join([get_cmd('des', 'node'), location, '| grep', work_field.current_pod])
                ...
                cmd_events = ' '.join([get_cmd('event'), '-n', work_field.ns, '-o json'])
        ...
        def switch_state(self):
            if work_field.current_page in ['log', 'pod', 'deployment', 'statefulset', 'daemonset']:
                ...
                work_field.content, work_field.content_c = work_field.content_c, work_field.content
                work_field.page_init(1, 1)
                return 'work'
            return 'wait'
            ...
            if work_field.current_page in work_field.page_level_2:
                ...
                if len(work_field.content) > 2:
                    if item == 'pod':
                        ...
                        for i in range(1,len(work_field.content)):
                            work_state = work_field.content[i].split()[1].split('/')
